...
// 1. Configura as chaves criptográficas (Ocultas nas variáveis de ambiente do Supabase)
webPush.setVapidDetails(
    'mailto:abrunos57@gmail.com', // Substitua pelo seu email de dev
    Deno.env.get('BHU3N0EE3mt78aRtRIhu_UXJsQGj6Ulu_0ZwEj5tgnO6NPIGDtyYaEDkiRP6XDWV93L4Jy2zKfQvhDI3zgil3WU')!,
    Deno.env.get('pWx5Fa4LaF_ub_VjKo1pQSuVKzwxoYgL-B4YD2ah0E4)!);
...
serve(async (req) => {
        try {
            ...
            if (payload.type === 'INSERT' && payload.table === 'agendamentos') {
                ...
                // 3. Conecta no Supabase como Administrador (Service Role) para buscar os celulares autorizados
                ...
                // 4. Dispara a notificação para todos os aparelhos do barbeiro (PC, Celular, etc)
                const promises = inscricoes.map(insc => {
                    return webPush.sendNotification(
                        insc.subscricao,
                        JSON.stringify({ titulo: 'ClientFlow ✂️', msg: mensagem })
                    ).catch(e => {
                        console.error("Erro ao enviar para um dispositivo (pode estar offline/inscricão expirada):", e);
                    });
                });
